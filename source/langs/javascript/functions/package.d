...
string jsOr(string[] conditions) {
	...
	foreach(condition; conditions) 
		if (condition.length > 0) 
			cs~= (condition.indexOf("(") == 0 ? condition : "("~condition~")"); 
	
	return conditions.length == 1 ? cs[0] : "("~cs.join("||")~")"; 
} 

string jsAnd(string[] conditions...) { return jsAnd(conditions.dup); }
